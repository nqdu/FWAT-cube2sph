...
#SBATCH --time=01:30:59
#SBATCH --job-name FWD_ADJ22
#SBATCH --output=FWD_ADJ22_%j.txt
...
if [ $set_num -le 10 ];then
  mpirun  -np $NPROC $fksem/bin/xfwat1_fwd_measure_adj $MODEL $SET noise 3
  #mpirun -oversubscribe -np $NPROC $fksem/bin/xfwat0_forward_data $MODEL $SET noise
else
  mpirun  -np $NPROC $fksem/bin/xfwat1_fwd_measure_adj $MODEL $SET tele 3
