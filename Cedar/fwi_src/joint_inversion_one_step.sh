...
for iter in `seq $iter_start $iter_end`;do 
  ...
  set_fwat1 $setb1 $sete1 $simu_type1
  job_adj1=$(sbatch tmp.fwat1.$simu_type1.sh|cut -d ' ' -f4 )

  # run RF/tele simulation
  set_fwat1 $setb2 $sete2 $simu_type2
  job_adj2=$(sbatch --dependency=afterok:${job_adj1} tmp.fwat1.$simu_type2.sh|cut -d ' ' -f4 )

  # set postprocessing file
  grep '^#'  sbash_fwat2_postproc_opt.sh  > tmp.fwat2.sh
  
  # save misfit for iter 0
  iter1=$iter
  iter=0
  if [[ "$iter" == "0" ]];then
    cat > tmp.fwat2.cal.sh << EOF
:> misfit0.log
...
for iset in \$(seq $setb2 $sete2); do echo "1.0 / \$mis2"|bc -l >> optimize/weight_kl.txt; done 
echo "\$mis1 \$mis2" > misfit0.log
EOF
    echo "bash tmp.fwat2.cal.sh" >> tmp.fwat2.sh
  fi
  iter=$iter1

  # run post 
  echo "postprocessing ..."
  grep -v '^#'  sbash_fwat2_postproc_opt.sh  >> tmp.fwat2.sh
  fwd=tmp.fwat2.sh
  sed -i "/MODEL=/c\MODEL=${mod}" $fwd
  sed -i "/SETB=/c\SETB=set${setb}" $fwd
  sed -i "/SETE=/c\SETE=set${sete}" $fwd 
  job_post=$(sbatch --dependency=afterok:${job_adj2} tmp.fwat2.sh | cut -d ' ' -f4)

  # run line search for noise 
  set_fwat3 $simu_type1
  #cat >> tmp.fwat3.$simu_type1.sh << EOF
...
#EOF
  ...
  # run line search for tele
  set_fwat3 $simu_type2
  #cat >> tmp.fwat3.$simu_type2.sh << EOF
#for f in output_fwat3*_${mod}_step*.txt; do mv "\$f" "\${f/${mod}/${mod}_${simu_type2}}"; done
#EOF
  job_line2=$(sbatch --dependency=afterok:${job_line1} tmp.fwat3.$simu_type2.sh | cut -d ' ' -f4)

  # set misfit file
  grep '^#' sbash_fwat4_opt_model.sh >  tmp.fwat4.sh 
  
  # compute misfit 
  cat > tmp.fwat4.cal.sh << EOF
  :> plots/$mod.mis.avg
...
EOF
  echo "bash tmp.fwat4.cal.sh " >> tmp.fwat4.sh
  grep -v '^#'  sbash_fwat4_opt_model.sh >> tmp.fwat4.sh
  fwd=tmp.fwat4.sh
  ...
  sed -i "/SETE=/c\SETE=set${sete}" $fwd
  echo "compute misfit ..."
  job_misfit=$(sbatch --dependency=afterok:${job_line2} tmp.fwat4.sh | cut -d ' ' -f4)
