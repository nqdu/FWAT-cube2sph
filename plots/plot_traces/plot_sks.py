...
def compute_ak135_time(evla,evlo,evdp,statxt):
    ...
    for i in range(nsta):
        ...
        arr = model.get_travel_times_geo(evdp,evla,evlo,stla,stlo,['SKS'])[0]
        t_ref[i] = arr.time
        dist[i] = arr.distance
...
def plot_event(line:str,M1:str,solver:str,outdir:str,band:str,
    ...
    for i in range(t_inj_txt.shape[0]):
        if t_inj_txt[i,0] == evtid:
            ...
            break 
    tref,dist = compute_ak135_time(float(info[1]),float(info[2]),
                            float(info[3]),statxt)
    tref = tref - t_inj 
    ...
    ax1.set_title("BXT before FWI")
    ax2.set_title("BXT after FWI")
