...
SOURCE_FILE=src_rec/sources.dat.ls.$simu_type 
iter=`echo $MODEL |cut -d"M" -f2 |awk '{printf "%d", $1}'`
nevts=`cat $SOURCE_FILE |wc -l`
work_dir=`pwd`
mod=$MODEL

fwd=output_fwat3_log.${MODEL}.$simu_type.job${SLURM_ARRAY_TASK_ID}.txt
:> $fwd 
for i in `seq 1 $NJOBS`; do
  ...
  evtdir=$work_dir/solver/$mod.ls/$evtid
  ...
  ln -s $work_dir/./optimize/MODEL_${mod}_step01/* $evtdir/$LOCAL_PATH/
  ...
  cd $work_dir
  echo ""
  bash $MEASURE_LIB/measure.$simu_type.sh $iter $evtid 2 >> $fwd 

  # delete useless information
  bash $FWATLIB/clean.sh $MODEL.ls $evtid 
