program main 
  ...
  ! filename
  character(len=256)  :: filename, outfile,tempfile,LOCAL_PATH,outfile_local 
  ...
  integer     :: nspec,nglob,i,j,k,ispec,iglob,nspec_ire,ierr 
  ...
  call MPI_Bcast(tempfile,256,MPI_CHARACTER,0,MPI_COMM_WORLD)
  call MPI_Bcast(outfile,256,MPI_CHARACTER,0,MPI_COMM_WORLD)
  call MPI_Bcast(LOCAL_PATH,256,MPI_CHARACTER,0,MPI_COMM_WORLD)

  ! open output file 
  write(outfile_local,'(a,a,i6.6,a)') trim(outfile),'.proc',rank,'.dat'
  open(IOUT,file=trim(outfile_local))
  ...
  if(rank == 0)then 
    ...
    write(cmd,'(a,a)') ':>',trim(outfile)
    call EXECUTE_COMMAND_LINE(cmd)
    do i=0,nproc-1
      print*,'copying ',i
      write(outfile_local,'(a,a,i6.6,a)') trim(outfile),'.proc',i,'.dat'
      write(cmd,'(a,a,a,a)')'cat ',trim(outfile_local),' >> ',trim(outfile)
      call execute_command_line(cmd)
      open(IIN,file=outfile_local)
