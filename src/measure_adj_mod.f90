...
subroutine measure_adj(data_in,syn_in,npts,t0,dt, &
                      fstart0,fend0,tstart,tend,tt,dtt,&
                      nn,chan_dat_in,chan_syn_in,chan_in,num_meas,&
                      datafile,synfile,net_in,sta_in,&
                      window_chi_out,tr_chi_out,am_chi_out,&
                      adj_syn_out)
  ...
  implicit none
  integer, intent(in) :: npts,nn,num_meas
  double precision, intent(in) :: dt,t0,tt,dtt
  double precision, intent(in) :: data_in(NDIM),syn_in(NDIM)
  character(len=*),intent(in) :: datafile,synfile,net_in,&
                                sta_in,chan_dat_in,chan_in,chan_syn_in
  double precision,value :: tstart,tend
  double precision,intent(out) :: window_chi_out(NCHI),tr_chi_out,am_chi_out
  double precision,intent(out) :: adj_syn_out(nn)

  character(len=150) :: synfile_phydisp,file_prefix,file_prefix0,file_prefix2,measure_file_prefix,adj_file_prefix
  integer :: j, ios, npt3
  double precision, dimension(NDIM) :: data, syn, syn_phydisp, adj_syn_all, &
                        tr_adj_src, am_adj_src, recon_cc_all, syn_dtw_cc, syn_dtw_mt
  double precision ::  t03, dt3,  df
  ...
  chan = chan_in
  chan_dat = chan_dat_in
  chan_syn = chan_syn_in
  ...
  do ipair = 1, 1
    ...
    do j = 1, num_meas
      ...
      !nqdu write(12,'(a3,a8,a5,a5,3i5,2f12.3)') net,sta,chan_syn,chan_dat,nwin,ipair,j,tstart,tend
      ...
      if (is_mtm == 1) then
        ...
        !  print *, '     period of max data/syn power    :', sngl(T_pmax_dat), sngl(T_pmax_syn)
         ...
         if (.not. use_trace) then
            ...
            !if(myrank == 0)print *, '   reverting from MT measurement to CC measurement...'
            imeas = imeas0 - 2
      ...
      if (COMPUTE_ADJOINT_SOURCE) then
         !if(myrank == 0)print *, '   Generating adjoint source and chi value for imeas = ', imeas
        ...
        !nqdu write(13,'(a14,a8,a3,a5,i4,i4,2e14.6,20e14.6,2e14.6,2f14.6)') &
   ...
   enddo ! nmeas
    ...
    ! write out the CC-reconstructed data from synthetics
      ...
      !if(myrank == 0) print *, 'writing adjoint source to file for the full seismogram'
