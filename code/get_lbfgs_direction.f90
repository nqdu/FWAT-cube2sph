...
!          i'm not sure if this makes a difference.
  ...
  nkernel_name(1) = "alpha_kernel_smooth"; nkernel_name(2) = "beta_kernel_smooth"
  nkernel_name(3) = "rhop_kernel_smooth"; nmodel_name(1) = "vp"
  nmodel_name(2) = "vs"; nmodel_name(3) = "rho"
  !nkernel_name=(/"alpha_kernel_smooth","beta_kernel_smooth","rhop_kernel_smooth"/)
  ...
  do istore=iter_current-1,iter_store,-1
     ...
     if (USE_NORM) then
       ...
       !if(myrank == 0) print*,'p_sum = ',p_sum !nqdu 
     endif
  ...
  do iker=1,NKERNEL
     ...
     do ispec=1,NSPEC
        do k=1,NGLLZ
         ...
         enddo
     enddo
     !if (USE_RHO_SCALING) then !nqdu errors with compiled with -fpe0
